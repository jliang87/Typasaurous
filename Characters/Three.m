...
//        self.state = UIGestureRecognizerStateFailed;
	...
	if(x == 2 && angle3 < -10 && angle3 >= -90 && currentPoint.y > previousPoint.y) //if moving down left
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 5)
			x = 3;
	...
	if (x == 4 && angle3 >= 0 && angle3 < 60 && currentPoint.x > previousPoint.x) //move straight right 
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 5)
			x = 5;
	...
	if (x == 6 && angle3 < -10 && angle3 >= -90  && currentPoint.y > previousPoint.y) // move down left
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
