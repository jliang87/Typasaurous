...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if((x == 0 && currentPoint.x < previousPoint.x && angle2 < 20 ) || (x == 0 && currentPoint.y > previousPoint.y && angle3 < 0 && angle3 >-40)) //if moving straight left
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 20)
			x = 1; //change state
	}
    else if(angle3 < -40 && angle3 >= -90 && currentPoint.y < previousPoint.y && x == 0)
	...
	if((x == 6 && currentPoint.y < previousPoint.y && angle3 > 0 && angle3 <= 70) || (x==6 && currentPoint.x < previousPoint.x && angle2 < 30)) //if moving up left
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 5)
			x = 7;
	}	
    	if(x == 7 && angle2 < 30 && currentPoint.x > previousPoint.x) 
            self.state = UIGestureRecognizerStateFailed;
}
