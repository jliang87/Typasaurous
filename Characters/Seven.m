...
//	
//	if(x == 0 && currentPoint.y < previousPoint.y && angle2 < 20) //if moving straight up
//	{
...
//	}
	...
	if(x == 0 && currentPoint.x > previousPoint.x && angle2 < 20) //straight right
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 20)
			x = 3; //change state
	}
    else if(currentPoint.y > previousPoint.y &&  angle3 < -40 && angle3 >= -90 && x == 0) 
	...
	if(angle3 < -10 && angle3 > -90 && currentPoint.y > previousPoint.y && x == 4) //if moving down left
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 20)
			x = 5;
	} 
    if (x == 5 && angle3 >= 0 && angle3 < 60 && currentPoint.x > previousPoint.x) 
        self.state = UIGestureRecognizerStateFailed;
