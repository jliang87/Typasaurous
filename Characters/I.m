...
#define kMaximumCheckMarkAngle    120
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if(x == 2 && angle2 < 30 && currentPoint.x < previousPoint.x) //if moving straight left
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 10)
			x = 3;
	} 
    else if (x == 2 && angle2 < 30 && currentPoint.x > previousPoint.x) //prevent from going straigh right
        self.state = UIGestureRecognizerStateFailed;
