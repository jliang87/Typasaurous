...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if(currentPoint.y > previousPoint.y && angle3 >= -90 && angle3 < -30 && x == 0) //if moving straight down
	{
...
//        self.state = UIGestureRecognizerStateFailed;
    ...
    if((x == 2 && currentPoint.y < previousPoint.y && angle3 < 0 && angle3 >= -90) )
	{
		lineLengthSoFar += distanceBetweenPoints(previousPoint, currentPoint);
		if (lineLengthSoFar > 20)
			x = 3;
	}
    else if (x == 2 && angle2 < 20 && currentPoint.x < previousPoint.x) 
        self.state= UIGestureRecognizerStateFailed;
//    else if(x == 2 && angle3 > 40 && angle3 < 80 && currentPoint.y > previousPoint.y) 
//        self.state = UIGestureRecognizerStateFailed;
